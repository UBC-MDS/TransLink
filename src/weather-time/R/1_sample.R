...
source("src/weather-time/R/helper_scripts/build_series.R")

all_data <- readRDS("data/weather-time/final_data.rds") %>%
  mutate(day_of_year = yday(loss_date))

stations_per_loc_hour <- readRDS("data/weather-time/stations_per_loc_hour.rds")
stations_per_loc_day <- readRDS("data/weather-time/stations_per_loc_day.rds")
...
write_csv(all_samples_combined, "data/weather-time/final_data_combined.csv")