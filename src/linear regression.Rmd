...
df <- df %>%
  mutate(Loss_day = weekdays(Loss_Date), Loss_year = year(Loss_Date),Loss_month = month(Loss_Date),
         Time_of_Loss =as.numeric(Time_of_Loss))
...
#df$Time_of_Loss.padded <- str_pad(df$Time_of_Loss, 4, pad="0")

#df$time.period <- hm(df$Time_of_Loss.padded)

df$Time_of_Loss <- format(strptime(sprintf('%04d', df$Time_of_Loss), format='%H%M'),'%H:%M')
