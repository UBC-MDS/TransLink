...
best_bayes_model <- readRDS("results/operators/best-bayes-model.rds")
posterior_predictions <- readRDS("results/operators/posterior_samples.rds")
results <- readRDS("results/operators/validation-results.rds")
...
operators_dataset <- read_csv("results/operators/train.csv") %>%
  sample_n(5) %>%
...
```{r, echo = FALSE}
img <- readPNG("results/claim_costs/joining_datasets.png")
grid.raster(img)
```
...
```{r include=FALSE}
my_data <- read_csv("results/claim_costs/collision_locations_with_coordinates.csv")
