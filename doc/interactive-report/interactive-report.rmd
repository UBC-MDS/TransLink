...
```{r include=FALSE}
target_location_df = read_csv("results/processed_data/collision_locations_with_coordinates.csv")
target_location_df <-target_location_df %>% drop_na(lat, long)
target_location_df$months <- month(as.POSIXlt(target_location_df$date, format="%Y-%m-%d"))
cwo <- target_location_df %>% filter(desc %in% c("Collisions With Objects", "Collisions With Objects (At G/C)"))
cwv <- target_location_df %>% filter(desc %in% c("Collisions With Other Vehicles", "Collisions With Vehicles (At G/C)"))
cwp <- target_location_df %>% filter(desc == "Collisions With People")
...
m <- leaflet() %>%
    ...
    addCircleMarkers(data = cwo,
                     stroke = FALSE, fillOpacity = 0.5, radius = 3,
                     color = ~pal(cwo$months),
                     popup=paste0("<b>", cwo$desc, "</b> <br>", 
                            "<b>", "Date: ", "</b> ", cwo$date, "<br>", 
                            "<b>","Bus Year: ", "</b> ", cwo$bus_year, "<br>", 
                            "<b>","Manufacturer: ", "</b> ", cwo$bus_manufacturer, "<br>" ),
                     group = 'Collisions With Objects') %>%
    addCircleMarkers(data = cwv,
                     stroke = FALSE, fillOpacity = 0.5, radius = 3,
                     color = ~pal(cwv$months), 
                     popup=paste0("<b>", cwv$desc, "</b> <br>", 
                            "<b>", "Date: ", "</b> ", cwv$date, "<br>", 
                            "<b>","Bus Year: ", "</b> ", cwv$bus_year, "<br>", 
                            "<b>","Manufacturer: ", "</b> ", cwv$bus_manufacturer, "<br>" ),
                     group = 'Collisions With Vehicles') %>%
    addCircleMarkers(data = cwp,
                     stroke = FALSE, fillOpacity = 0.5, radius = 3,
                     color = ~pal(cwp$months), 
                     popup=paste0("<b>", cwp$desc, "</b> <br>", 
                            "<b>", "Date: ", "</b> ", cwp$date, "<br>", 
                            "<b>","Bus Year: ", "</b> ", cwp$bus_year, "<br>", 
                            "<b>","Manufacturer: ", "</b> ", cwp$bus_manufacturer, "<br>" ),
                     group = 'Collisions With People')%>%
  leaflet::addLayersControl(overlayGroups = c("Collisions With Objects", "Collisions With Vehicles", "Collisions With People"),
                   options = layersControlOptions(collapsed = FALSE)) %>%
  addLegend(
    data = target_location_df,
    "bottomleft", 
    pal = pal,
    values = ~months,
    opacity = .9,
...
calculate_plotting_stats <- function(col_num) {
  ...
  plotting_data <- tibble(
      ...
      Estimate = round(apply(data_re, 2,  FUN = median), 3),
      `Lower 5%` = round(apply(data_re, 2, FUN = function(x) quantile(x, 0.05)), 3),
      `Upper 95%` = round(apply(data_re, 2, FUN = function(x) quantile(x, 0.95)), 3)
    ) %>%
    mutate(`Cost Centre` = fct_reorder(`Cost Centre`, Estimate, min))
  if (col_num == 1) {
    pop_effect_opt <- tibble(`Global Effect` = round(median(pop_effect[, 1]), 3))
  } else {
    pop_effect_opt <- tibble(`Global Effect` = round(median(pop_effect[, 1] + pop_effect[, col_num]), 3))
  }
...
output$fe_plot <- renderPlotly({
  ...
  if (cost_centre_sel != "All") {
    effects_per_centre <- pop_effect + random_effects[, cost_centre_sel, ]
  } else {
    intercept <- pop_effect[, 1]
    effects_per_centre <- cbind(intercept, (pop_effect + intercept)[, 2:4])
  }
  plot_cost_sel <- tibble(
    Estimate = round(apply(effects_per_centre, 2, median), 3),
    Experience = as.factor(c(">60 Months", "<6 Months", ">18 & <60 Months", ">6 Months & <18 Months")),
    `Lower 5%` = round(apply(effects_per_centre, 2, function(x) quantile(x, 0.05)), 3),
    `Upper 95%` = round(apply(effects_per_centre, 2, function(x) quantile(x, 0.95)), 3)
  ) %>%
  ...
  geom_point() + 
  geom_errorbar(aes(ymin = `Lower 5%`, ymax = `Upper 95%`)) + theme_economist() +
  labs(x = "Experience Level", y = "Estimated Effect (log scale)"), width = 800, autosize = TRUE) %>%
  layout(title = list(text = paste0("Estimated Effect of Experience for Cost Centre: ", cost_centre_sel), x = 0)) %>%
...
output$re_plot <- renderPlotly({
  ...
  ggplotly(ggplot(plotting_data[[1]], aes(x = `Cost Centre`, y = Estimate, ymin = `Lower 5%`, ymax = `Upper 95%`)) +
    geom_point() + 
    geom_errorbar(aes(ymin = `Lower 5%`, ymax = `Upper 95%`), width = 0.1) +
    geom_hline(data = plotting_data[[2]], aes(yintercept = `Global Effect`), color = "red") + 
    coord_flip() +
    labs(y = "Estimated Effect (log scale)") + 
    theme_economist() +
