...
calculate_plotting_stats <- function(col_num) {
  ...
  data_re <- as_tibble(random_effects[, , col_num] + pop_effect[, col_num])
  plotting_data <- tibble(
    ...
    pop_effect_opt <- tibble(`Global Effect` = round(median(pop_effect[, col_num]), 3))
...
tabsetPanel(
  ...
  tabPanel("Fixed Effects", plotlyOutput("fe_plot"))
)

output$fe_plot <- renderPlotly({
  
  ggplotly(ggplot(marginal_plot, aes(x = Effect, y = Estimate, ymin = Lower, ymax = Upper)) +
  geom_point() + 
  geom_errorbar(aes(ymin = Lower, ymax = Upper)) + theme_economist() +
  labs(x = "Experience Level", y = "Estimated Incidents/Year"), width = 800, autosize = TRUE) %>%
  layout(title = list(text = "Estimated Incidents/Yr Over All Cost Centres", x = 0)) %>%
