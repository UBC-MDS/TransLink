...
<center> <h4>Relevant Terminology</h4> </center>
...
TransLink has seen their insurance premium to ICBC increase by over 200% in response to an increase in onboard passenger injuries, cyclist injuries, pedestrian injuries, and losses from collisions with third party vehicles. In addition, their premium has increased by roughly 33% to their own captive insurance company. As a result, this report seeks to identify specific variables that are highly predictive of bus incidents so that TransLink can make data driven decisions to help reduce the number of incidents (and therefore, claims). In addition, this report provides two interactive dashboards that seek to analyze common types, and the overall cost impact, of claims resulting from these incidents. Using these dashboards, TransLink can hopefully assess in finer detail what is actually driving up their insurance costs.

Overall, this report is divided into four sections in which the first two sections deal primarily with the analysis of variables that may be linked to the frequency of bus incidents. The last two sections analyze data related to the actual claims that result from these incidents - in particular, claim descriptions and claim costs. The first section deals with an analysis of bus operator characteristics which is what TransLink currently uses to assess incident risk. In this section, both experience level and operator cost centre are analyzed in detail to determine the effect (if any) these variables have on the number of incidents per year an operator is involved in. Using only this information, results show that both experience level and operator cost centre are highly predictive of the number of incidents per year an operator has, in which operators who are more experienced achieve lower incidents per year over all cost centres. However, Port Coquitlam is noted to be an outlier cost centre in which more experienced operators (with less than five years of experience) are involved in **more** incidents per year. It is recommended that operators with less than five years of experience in Port Coquitlam are analyzed in further detail to better understand why this is the case.

The second section can be viewed as an extension of the first one, with the key difference being that weather, time, bus line, and bus characteristic variables are used in addition to the operator characteristics previously mentioned. The methods used here incorporate a state of the art machine learning approach to assess the relative risk of any scenario described by these variables. Through interpretation of the models outputs, it is found that the bus line is by far the most important variable in determining whether an incident occurs or not. In particular, it is found that bus lines that move through Downtown Vancouver are estimated to be the riskiest - *8 out of the top 10 riskiest bus lines all move through Downtown Vancouver*, and these bus lines account for roughly 23% of all incidents since 2011. Therefore, if TransLink focuses solely on reducing the number of incidents for all bus lines that operate through Downtown Vancouver, it is believed that TransLink can mitigate a large portion of risk. Other notable results in this analysis include buses greater than 24 years of age being of significantly higher risk of incidence, and temperatures in excess of 31°C/rain amounts in excess of 18 mm yielding notable spikes in risk.

The third section analyzes manually written claim descriptions that in general, tend to describe the actual incident that led to each individual claim itself. Each claim is displayed on an interactive map that shows where the actual incident took place, allowing the user to spot any potential patterns in the types of incidents for any specific area. One type of claim that is immediately noticeable are those that involve mirrors. It would appear that many claims involve the replacement of bus mirrors and this appears to be a noteworthy area of the bus that is commonly damaged in incidents.

Finally, the last section analyzes the actual claim costs associated with each incident (if any), in a similar fashion to what is done in the third section. A map is provided which details where each incident took place and the user can view information regarding the claim costs associated with any incident on the map. Detailed statistics on the left side of the dashboard are also provided which can be helpful as an aid when analyzing the actual claim costs per different variables of interest. In the end, it is observed that a large portion of claim costs are coming from a greater frequency of minor incidents rather than say, a smaller number of very severe incidents. Combining this with the third section above, it is reasonable to think that a large portion of these minor claims could simply be damage to bus mirrors.
...
Note that the risk curves are fit using a secondary regression method to the risk scores predicted by the machine learning model (to more easily see the relationship graphically). Therefore, the averages produced by the slider and the averages produced using the risk curve may not be the same, but in general they should be quite close. If there is a discrepancy, we recommend trusting the curve rather than the averages produced from the slider input.
...
Overall, we can see that the lines estimated to be of highest risk for an incident are the 22, 10, 41, 20, 14 and the 4. All of these lines with the exception of line 41 go through Downtown Vancouver, in fact, eight out of the top ten riskiest bus lines move through Downtown Vancouver and account for about 23% of all bus incidents since 2011. It is difficult to know exactly why Downtown is such a troublesome area - perhaps more traffic and overall population density could play a role. Indeed, we see in the spatial analysis section that Downtown Vancouver dominates the total number of claims as well. 
...
- 7 am and 6 pm are the times that are most risky for incidents, however, within this range there are distinguishable intervals that are relatively safer. For example, the morning rush hour (7 am to 9 am) is roughly 1.1 times less likely to have an incident when compared to 3 pm to 5 pm. 
...
<center> <h4>Claim Descriptions by Location</h4> </center>
...
The dashboard below analyzes the manually written claim descriptions filled out by TransLink employees. The following list displays five randomly selected claim descriptions:
...
<center> <h4>Difficulties Analyzing Claim Costs</h4> </center>  
...
<center> <h4>Methodology and Usage</h4> </center>  
...
server2 <- function (input, output) {
  ...
  output$table1 <- renderTable(spacing = c('xs') ,{
    ...
    updated_data %>% group_by(line_no) %>% summarise(total_cost_line = sum(`paid_cost$`)) %>% 
      data.frame()%>%  drop_na() %>% arrange(desc(total_cost_line)) %>% rename(c("Line No." = "line_no" , "Total Cost" ="total_cost_line" )) %>% head(5) %>% mutate(`Total Cost` = paste('$',formatC(`Total Cost`, big.mark=',', format = 'f', digits = 0, drop0trailing = TRUE)))
  })
  ...
  output$table2 <- renderTable( spacing = c('xs') ,{
    ...
    data.frame() %>% drop_na() %>% arrange(desc(total_cost_line)) %>%  rename(c("Bus Manufact." = "asset_manufacturer", "Total Cost" = "total_cost_line" )) %>% head(5) %>% mutate(`Total Cost` = paste('$',formatC(`Total Cost`, big.mark=',', drop0trailing = TRUE, digits = 0, format = 'f')))
  })
  ...
  output$table3 <- renderTable( spacing = c('xs') ,{
    ...
    data.frame() %>% drop_na() %>% arrange(desc(n)) %>%  rename(c("City of Incident" = "city_of_incident", "Number of Incidents" = "n" )) %>% head(5)
  })
...
```{r , echo = FALSE}
shinyApp(ui2, server2, options = list(height = 915))
```
