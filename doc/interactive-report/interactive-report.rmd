...
We employed a machine learning model to simulatenously assess the overall predictive power of all of these various factors using a very similar methodology to that of [@Montreal] and [@WilsonML]. Full details are described in the final written report that is also submitted with this one. On average, the model shown here is expected to correctly rank a scenario with an actual incident to be of higher risk than a scenario without, 85% of the time. This is is line with the papers in which this model is based on.
...
As an example, we see that line  
