...
ui <-  fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
     ...
     mainPanel(
       fluidRow(column(3, plotOutput("plot1", width = "100%", height = "200px") ),
                column(3, plotOutput("plot2", width = "100%", height = "100px") ) )
...
server <- function (input, output) {
  ...
  observe({
    ...
    experience <- input$experience
      pal <- colorFactor(palette = c("turquoise", "steelblue4","sienna1", "violetred1" ), levels = c("Level1 (<$200)", "Level2 ($200-$1.000)", "Level3 ($1.000-$10.000)", "Level4 (>$10.000)"))
  ...
  output$plot2 <- renderPlot({
    updated_data <- year_data() %>%
      ...
      p <- updated_data %>% group_by(cost_range) %>%
           summarise(prop_cost_range =n()/ n) %>%
           ggplot(aes(x = "" , y = prop_cost_range, fill = cost_range)) +
           geom_bar(stat= "identity") + coord_polar( "y", start = 0) +
           geom_text(aes(label = paste0(round(prop_cost_range*100), "%")), position = position_stack(vjust = 0.5), color = "white")+theme_void()+                    scale_fill_manual(values=c("turquoise", "steelblue4", "sienna1", "violetred1" ))
      p
  })
  ...
  output$plot1 <- renderPlot({
    ...
    updated_data %>% filter(bus_age > -1  & bus_age < 30 ) %>%
      ggplot(aes(bus_age)) +geom_density(fill = 'lightblue') +
      labs(title = "The Distribution of Bus Ages",
           x = 'Bus Age',
           y = 'Number of Incidents') + theme_bw()
