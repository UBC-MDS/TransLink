...
Note that the risk curves are fit using a secondary regression method to the risk scores predicted by the machine learning model (to more easily see the relationship graphically) and therefore there are regression. Therefore, the averages produced by the slider and the averages produced using the risk curve may not be the same, but in general they should be quite close. If there is a discrepancy, we recommend trusting the curve rather than the slider input.
