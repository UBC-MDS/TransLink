...
One of the primary concerns for TransLink was soaring insurance costs in recent years. Concerning the insurance costs TransLink interested in causal/exploratory analysis as well as cost prediction for upcoming months. However, after analyzing the available data-set, we figured out that almost half of the claims are still open and there were no negative examples in which the bus completed its route without any incident, hence without any cost. A predictive machine learning model on the insurance costs would be potentially highly biased since the insurance costs cannot be assessed accurately. Therefore, the best approach to get more insights into the increasing insurance costs was using exploratory data analysis techniques. For that purpose, we developed an interactive map tool to easily check and explore the spatial details of the collisions on a real city map. As the first step in our analysis, we combined different datasets to get more information about the claims, collisions, and also operators. Since there are many variables from different datasets that might have an effect on the insurance costs, we tried to build an extended dataset by joining the `2020 Collisions- Preventable and Non Preventable UBC Set Without Claim Number.csv`, `employee_experience_V2.csv` and also `claim_vehicle_employee_line.csv` datasets.

![](../../images/joining_dataset_2.png)

On the top of combined dataset, we populated the coordinates of each incident by using Google Maps GeoCoding API to be able to visualize them on the map. Furthermore, we also used an R package `leaflet` to add interactivity on this map.

Since our primary focus in this analysis is the insurance costs, we identified four levels of costs with input from TransLink to show the distribution of cost levels under specific conditions. According to the previous analysis, *time* and *the experience level of the operators* are two of the most apparent variables that affect the incidents. Therefore we used these two variables as first-level filter elements in our interactive dashboard.

Our primary element in the dashboard is the interactive map in the middle, which contains all reported incidents in the city map as a point, colored differently to indicate which cost level they belong to. In addition to the map, we also provided summary information that could help understand the data better. To achieve good coverage, we have used a combination of 
...
On the left, there are control elements that can be used for filtering the data by incident year and operator experience. On the right side, there is an interactive map that allows users to zoom in/out and selecting an individual incident which then shows a popup including details about the incident. The tile controls on the right corner allow users to change map tiles such as switching from street view to details view. 
...
2. The bus age distributions look quite similar for all experience groups and the cost levels. The distributions are most cumulated around 10.
...
4. Although most of the incidents have occurred in Vancouver and Burnaby for all experience groups, the order of the following more risky cities is different for each experience level. For example, the third, fourth, and fifth risky cities are Surrey, Richmond, and North Vancouver for the group with more than 60 months of experience. On the other hand, for a group with less than 6 months' experience, the order is North Vancouver, Richmond, and Coquitlam.
...
Things to be improved: The map shows the areas with the most incidents, but it is hard to use the phrase “most dangerous”, as the dataset does not tell us how many buses traveled through each area without any incidents.  